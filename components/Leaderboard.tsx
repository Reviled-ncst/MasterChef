...
export default function Leaderboard({ initialData }) {
  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem', position: 'relative', zIndex: 2 }}>
        <h1 style={{ marginBottom: 16, color: PRIMARY }}>Leaderboards</h1>

        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
          <input placeholder="Search by name or country" value={query} onChange={(e) => setQuery(e.target.value)} style={{ flex: 1, padding: '8px 10px', borderRadius: 6, border: '1px solid #ddd' }} />

          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} style={{ padding: '8px', borderRadius: 6 }}>
            <option value="rank">Sort: Rank</option>
            <option value="points">Sort: Points</option>
            <option value="achievements">Sort: Achievements</option>
          </select>
          <select value={sortDir} onChange={(e) => setSortDir(e.target.value as any)} style={{ padding: '8px', borderRadius: 6 }}>
            <option value="desc">Desc</option>
            <option value="asc">Asc</option>
          </select>

          <input type="number" placeholder="Min points" value={minPoints ?? ''} onChange={(e) => setMinPoints(e.target.value === '' ? undefined : Number(e.target.value))} style={{ width: 110, padding: '8px', borderRadius: 6 }} />
          <input type="number" placeholder="Min achv" value={minAchievements ?? ''} onChange={(e) => setMinAchievements(e.target.value === '' ? undefined : Number(e.target.value))} style={{ width: 110, padding: '8px', borderRadius: 6 }} />

          <button onClick={() => setViewMode(viewMode === 'cards' ? 'table' : 'cards')} style={{ padding: '8px 12px' }}>{viewMode === 'cards' ? 'Table' : 'Cards'}</button>
          <button onClick={() => { setQuery(''); setMinPoints(undefined); setMinAchievements(undefined); setSortBy('rank'); setSortDir('desc'); }} style={{ padding: '8px 12px' }}>Reset</button>
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'right', padding: 8 }}>Achievements</th>
                ...
                <tbody>
                  {items.map((p) => (
                    <tr key={p.id}>
                      <td style={{ padding: 8 }}>{p.rank}</td>
                      <td style={{ padding: 8 }}>
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                          <img src={p.avatar || FALLBACK_AVATAR} alt={p.name} style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover' }} />
                          <span style={{ fontWeight: 700, color: MUTED_TEXT }}>{p.name}</span>
                        </div>
                      </td>
                      <td style={{ padding: 8, textAlign: 'right' }}>{p.points?.toLocaleString()}</td>
                      <td style={{ padding: 8, textAlign: 'right' }}>{p.achievements}</td>
                      <td style={{ padding: 8 }}>{p.country}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
